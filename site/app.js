...
function q10(){choices('About who are you spinning around?',[{key:'q10',label:'Heterosexual',reaction:'Good to know.',next:q11},{key:'q10',label:'Bisexual',reaction:'Good to know.',next:q11},{key:'q10',label:'Homosexual',gameOver:true},{key:'q10',label:'Other',gameOver:true},{key:'q10',label:"Let's fuuuck.",next:secretVictory}],{subtitle:'Your orientation.'})}
function q11(){choices('Would you rather...',[{key:'q11',label:'Regret trying',reaction:"That's the spirit.",icon:'🔥',next:q12},{key:'q11',label:'Regret never trying',gameOver:true}])}
...
(()=>{
  ...
  const resize=()=>{dpr=Math.min(devicePixelRatio||1,2);w=innerWidth;h=innerHeight;c.width=w*dpr;c.height=h*dpr;c.style.width=w+'px';c.style.height=h+'px';x.setTransform(dpr,0,0,dpr,0,0);stars=Array.from({length:Math.min(155,Math.floor(w*h/7200))},()=>({x:Math.random()*w,y:Math.random()*h,r:Math.random()*1.45+.3,a:Math.random()*.72+.22,p:Math.random()*6.28,v:Math.random()*.045+.012}))};
  const spawn=t=>{if(!shoot&&Math.random()<.0026)shoot={x:Math.random()*w*.7,y:Math.random()*h*.35,len:80+Math.random()*100,life:0}};
  const draw=t=>{x.clearRect(0,0,w,h);for(const s of stars){s.y-=s.v;if(s.y<0)s.y=h;const p=.62+.38*Math.sin(t*.001+s.p);x.beginPath();x.arc(s.x,s.y,s.r*p,0,Math.PI*2);x.fillStyle=`rgba(225,230,255,${s.a*p})`;x.fill()}spawn(t);if(shoot){shoot.life+=.035;const a=Math.max(0,1-shoot.life);x.strokeStyle=`rgba(235,238,255,${a*.82})`;x.lineWidth=1;x.beginPath();x.moveTo(shoot.x,shoot.y);x.lineTo(shoot.x+shoot.len,shoot.y+shoot.len*.42);x.stroke();shoot.x+=5;shoot.y+=2.1;if(shoot.life>=1)shoot=null}requestAnimationFrame(draw)};addEventListener('resize',resize);resize();requestAnimationFrame(draw)
})();
initializeGame();
